...
This continues the EDA process. In part 1A we explored the data distribution of the dataset. Here in 1B we will explore the covariation between input and output. 
- Covariation between an input and an output could indicate the input can be used to predict the output. 
- Visualize the relationship between input and output to help with feature engineering. 
- Examine if interactions could affect prediction. 


Load and transform the data as in Part1A. 
...
```{r out.width=1000, fig.width=10, fig.asp=0.65, warning=FALSE}
...
- There are correlations between many inputs and the `response`. 
  - Indicate these inputs should be able to predict the `response`. 
  - Since we'll log-transform the `response` to remove its lower bound, will examine again at log-transformed scale. 
...
- It appears the categorical inputs have impact on the `response`. For example:
  - The average `response` in region `ZZ` is clearly less than that in region `XX, YY`.
  - Customer `M` received least average time spent on sale than other customer groups. 
  - Customer `D` received on average the most sale time. 

**For each region, see how `response` differ across customers. **
...
- At each region, `response` differs by `customer`. For example:
...
- This basically reflects how the `response` correlates to `region` in a prevoius plot. 


### Response vs Continuous and Categorical Inputs

**Log-transformed response from each continuous input, conditioned on region. **
...
- Region does have affect the relationship between certain inputs and the `response`. For example
  - The `region` totally changes the direction of the correlation between inputs `xa_04`, `xa_05`, `xa_08`, `xs_01`, `xs_02` and `response`.
    - This suggests that at different `region`, the same input feature could have contrasting effect on `response`. [interpretation]
  - Consider interact `region` to the inputs. [feature-engineering]

**Log-transformed response from each continuous input, conditioned on customer. **
...
- Just like `region`, `customer` also affects how certain inputs could predict the `response`. 
  - Consider interact `customer` to the inputs. [feature-engineering]
...
- As shown by the smoother, some inputs have clear relation to the binary `outcome`.
  - For most inputs, low input ranges associate with higher event probability, while high input ranges correlate with lower event probability.
  - This is not true for `xs_04`,`xs_05`,`xs_06`, and all `xw` input features, which does not seem to affect event probability too much. 
  
**Examine how the response affects outcome.**

```{r out.width=800, fig.width=8, fig.asp=0.40, warning=FALSE}
df_eda %>%
...
```
- The `response` does have some impact on `outcome`. 
...
```{r out.width=800, fig.width=8, fig.asp=0.30, warning=FALSE}
df_eda %>% select(c(sel_cat,"outcome","binary_outcome")) %>%
...
- It appears that `region` and `customer` does have impact on event probability. For example:
  ...
  - Consider interacting the two categorical inputs to predict `outcome`. [feature-engineering] 

### Outcome vs Continuous and Categorical Inputs
...
plot_grid(p)
```
- The `region` changes how each input could predict the `outcome`. 
...
- Similarly, the `customer` does affect how an input could predict event probability.
