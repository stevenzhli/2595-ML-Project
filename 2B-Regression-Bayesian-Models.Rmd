...
- Model `mod_2B2` has the lowest posterior σ mode (green dashed line). 
...
- For the best model identified using WAIC `mod_2B3`:
  ...
  - The corresponding MLE estimated σ is 0.292 (blue dotted line).
  - The MLE underestimated σ: there is 81.2% chance σ is greater than the MLE estimate, judging from the posterior samples. 

```{r}
sprintf("Posterior mode σ: %.3f", sigma(mod_2B3))
sprintf("MLE estimated σ: %.3f", sigma(mod_2AC5))

my_mod_sigma <- mod_2B3 %>% as_tibble() %>% select(sigma)
sprintf("Probability of posterior σ > MLE σ: %.3f%%", mean(my_mod_sigma >= sigma(mod_2AC5))*100)
```
