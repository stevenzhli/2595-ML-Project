...
- Use weak prior of R-squared 0.5. 
...
```{r}
mod_2B1_A3 <- readr::read_rds("models/mod_2B1_A3")
mod_2B2_Ac1 <- readr::read_rds("models/mod_2B2_Ac1")
mod_2B3_Ac3 <- readr::read_rds("models/mod_2B3_Ac3")
```
...
```{r}
mod_2B1_A3$waic <- waic(mod_2B1_A3)
mod_2B2_Ac1$waic <- waic(mod_2B2_Ac1)
mod_2B3_Ac3$waic <- waic(mod_2B3_Ac3)
```

```{r}
my_models <- stanreg_list(
  mod_2B1_A3, mod_2B2_Ac1, mod_2B3_Ac3,
  model_names = c("2B1_A3, all linear","2B2_Ac1, add nonlinear","2B3_Ac3, region: non-linear"))
loo_compare(my_models, criterion = "waic")
```

- Based on WAIC, model `mod_2B3_Ac3` with interaction from `region` to nonlinear features is the best model.
...
```{r}
bayes_R2(mod_2B3_Ac3) %>% quantile(c(0.05, 0.5, 0.95))
```
...
plot(mod_2B3_Ac3, pars = names(mod_2B3_Ac3$coefficients)) +
...
mod_2B3_Ac3 %>% as_tibble() %>% 
  select(all_of(names(mod_2B3_Ac3$coefficients))) %>% 
...
- Visualize the σ posterior distribution across all 3 models. 
...
my_models %>% pull_bayes_posterior_sigma() %>% 
  ...
  geom_vline(xintercept = sigma(mod_2B3_Ac3), color="#4D9EFF", linetype="dashed", size=0.5)
```

- Model `mod_2B3_Ac3` has the lowest posterior σ mode (blue dashed line). 
...
- For the best model identified using WAIC `mod_2B3_Ac3`:
...
```{r}
sprintf("Posterior mode σ: %.3f", sigma(mod_2B3_Ac3))
sprintf("MLE estimated σ: %.3f", sigma(mod_2Ac1))

my_mod_sigma <- mod_2B3_Ac3 %>% as_tibble() %>% select(sigma)
