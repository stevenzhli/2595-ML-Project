...
For my best regression model, the most important feature is the `xw_01`, and the most unpredictable customer is `K`, which is only present in region `ZZ`.  

```{r}
my_inputs <- setdiff(names(df_regr),"response")
viz_grid <- make_test_input_grid(my_inputs, df_regr, c("xw_01")) %>% mutate(customer = "K", region = "ZZ")
```
