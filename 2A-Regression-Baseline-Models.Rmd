...
**2. Add interaction from region and customer, keep the rest as 2Ac1.**

```{r model 2Ac2}
mod_2Ac2 <- lm(
    response ~ region * customer + . - region - customer
    + splines::ns(xa_03,df=2) + splines::ns(xa_05,df=2) + splines::ns(xa_08,df=2)
    + splines::ns(xb_03,df=2) + splines::ns(xb_05,df=2) + splines::ns(xb_08,df=2)
    + splines::ns(xn_03,df=2) + splines::ns(xn_05,df=2) + splines::ns(xn_08,df=2)
    + splines::ns(xs_01,df=2) + splines::ns(xs_04,df=2) + splines::ns(xs_06,df=2)
...
- Special case
  - The all-pairwise model (`mod_2Ac4`) resulted negative AIC/BIC. I talked to Dr. Yurko, and he suggests skipping this one for the Bayesian analysis because it will have enormous uncertainty, but including it for resampling in 2D).  
...
```{r}
pull_lm_coefs_signif(mod_2A3, 0.95) %>% knitr::kable()
pull_lm_coefs_signif(mod_2Ac1, 0.95) %>% knitr::kable()
pull_lm_coefs_signif(mod_2Ac3, 0.95) %>% knitr::kable()
```

- For continuous inputs, potential important ones are `xw_01`, `xb_04`, `xb_07`, `xa_02`.  
- For categorical inputs, it depends on the continuous features:
  - If we use the unmodified continuous inputs (`mod_2A3`), both the `region`, `customer` are important.  
  - As mentioned, once we add nonlinear features (`mod_2Ac1`), the `region` coefficients become more uncertain.  
  - Once we interact `region` to the nonlinear features, it is notable that interactions from `region` are important.  
