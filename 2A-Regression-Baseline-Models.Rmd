...
```{r}
pull_lm_coefs_signif(mod_2A3, 0.95)
pull_lm_coefs_signif(mod_2Ac1, 0.95)
pull_lm_coefs_signif(mod_2Ac3, 0.95)
```
...
- For categorical inputs, it depends on the continuous features:
  - If we use original continuous inputs (`mod_2A3`), both the `region`, `customer` are important. 
  - As mentioned, when nonlinear features were added (`mod_2Ac1`), the `region` coefficients become more uncertain.  
