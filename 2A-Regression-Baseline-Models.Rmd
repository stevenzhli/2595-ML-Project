...
- `mod_2A3`: Linear additive relationship of all continuous and categorical inputs got best BIC.  
...
**2. Remove additive from categorical inputs, keep the rest as 2Ac1.**
...
- Special case
  - The all-pairwise model (`mod_2Ac4`) resulted large negative AIC/BIC. I talked to Dr. Yurko, and he suggests skipping this one for the Bayesian analysis because it will have enormous uncertainty, but including it for resampling/regularization in part 2D.  
  - Thus I will skip it here too, and examine only models with second lowest AIC, second lowest BIC, and one in between for both metrics.
...
- For categorical inputs, the `region` appears important, and some levels of `customer` too.
