...
In section 1A we will explore the data for basic properties and distribution of the variables. 
...
```{r out.width=400, fig.width=4, fig.asp=0.45, warning=FALSE}
df_eda %>% select(sel_cat) %>% pivot_longer(everything()) %>%
  ggplot() +
...
- All sentiment derived features are numeric.
  - They are at different scales, and need to be normalized. [preprocessing]
...
- Most features look normally distributed, except:
  ...
  - The `xs_05` looks like it has 0 as lower bounded.
- The four numeric categorical inputs are manifested by the separated bins found on `xb_02`,`xb_03`,`xn_02`,`xn_03`.
